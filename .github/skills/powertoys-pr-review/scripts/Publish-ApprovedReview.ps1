...
function Assert-PendingReview {
    ...
    for ($index = 0; $index -lt $actualComments.Count; $index++) {
        ...
        foreach ($propertyName in @('path', 'body')) {
            if ([string]$actual.$propertyName -ne [string]$expected.$propertyName) {
                throw "PR $($Plan.number) pending comment $index changed property $propertyName during GitHub rendering (expected '$($expected.$propertyName)', actual '$($actual.$propertyName)')."
            }
        }

        $usesLegacyPosition = (
            $null -eq $actual.line -and
            [int]$actual.position -gt 0 -and
            [string]$actual.commit_id -eq [string]$Plan.headSha -and
            [string]$actual.original_commit_id -eq [string]$Plan.headSha
        )
        if (-not $usesLegacyPosition) {
            $actualLine = if ($null -ne $actual.line) { [string]$actual.line } else { [string]$actual.original_line }
            if ($actualLine -ne [string]$expected.line) {
                throw "PR $($Plan.number) pending comment $index changed property line during GitHub rendering (expected '$($expected.line)', actual '$actualLine')."
            }

            $actualSide = [string]$actual.side
            if ([string]::IsNullOrWhiteSpace($actualSide) -and $null -eq $actual.line -and $null -ne $actual.original_line) {
                $actualSide = 'RIGHT'
            }
            if ($actualSide -ne [string]$expected.side) {
                throw "PR $($Plan.number) pending comment $index changed property side during GitHub rendering (expected '$($expected.side)', actual '$actualSide')."
            }

            $expectedStartLine = if ($null -eq $expected.start_line) { '' } else { [string]$expected.start_line }
            $actualStartLine = if ($null -ne $actual.start_line) { [string]$actual.start_line } elseif ($null -ne $actual.original_start_line) { [string]$actual.original_start_line } else { '' }
